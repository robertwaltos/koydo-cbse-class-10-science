...
import { EXAM_CONFIG } from "@/lib/cbse-class-10-science/config";

describe("CBSE Class 10 Science Config", () => {
  it("has correct exam ID", () => {
    expect(EXAM_CONFIG.examId).toBe("EXAM186");
  });

  it("has correct slug", () => {
    expect(EXAM_CONFIG.slug).toBe("cbse-class-10-science");
  });
